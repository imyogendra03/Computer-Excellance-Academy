...
const DppTab = ({ subjects = [] }) => {
  const groups = subjects.flatMap((subject) =>
    subject.chapters.map((chapter) => ({
      subjectTitle: subject.title,
      chapterTitle: chapter.title,
      items: chapter.items.filter((item) => item.type === "dpp"),
    }))
  );
                    ...
                    <div className="mb-4 d-flex gap-3">
                      ...
                      <div>
                        ...
                        <p className="small text-secondary mb-0 opacity-75">
                          {item.description || "Practice problems for this chapter."}
                        </p>
                      </div>
                    </div>
      ...
      {groups.every(g => !g.items.length) && (
        <div className="col-12">
          <div className="lms-empty-state text-center py-5 rounded-5 border border-dashed border-secondary border-opacity-50">
            ...
            <h3 className="h4 opacity-75">DPP Coming Soon</h3>
            <p className="text-secondary opacity-50">
              Practice sheets are being prepared.
            </p>
          </div>
        </div>
      )}
